...
The project uses `cppcheck` for static code analysis both locally and in GitHub Actions CI.
...
- `-i code/build` excludes generated build files and reduces unnecessary warnings
- `--inline-suppr` enables the small number of source-level suppressions used for known false positives
- `--error-exitcode=1` makes the command fail when unsuppressed findings are reported
