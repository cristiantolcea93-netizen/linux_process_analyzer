...
static void run_snapshot_once(const int *filter_pids, size_t filter_pids_count);
...
static void run_snapshot_once(const int *filter_pids, size_t filter_pids_count)
{
	collect_snapshot(filter_pids, filter_pids_count);
}
			...
			if(tfd != -1)
			{
				...
				while(noOfIterations < gArguments.count)
				{
					...
					while(expirations-- && noOfIterations < gArguments.count)
					{
						...
						run_snapshot_once(gArguments.filter_pids, gArguments.filter_pids_count);
					}
					noOfIterations++;
				}
				//deinit snapshot
